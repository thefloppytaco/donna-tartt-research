#!/usr/bin/env bash
# Wayback Machine sweep v3 — CDX-only, robust to throttling
#
# Changes from v2:
# - Drops the availability API (it 429s aggressively); uses CDX exclusively.
# - Strict timestamp validation: only [0-9]{14} is ever passed to fetch_snapshot,
#   so the v2 "WBRATE" homepage-as-capture bug cannot recur.
# - CDX query has 3-attempt backoff (30s, 60s, 120s) on non-200 or non-JSON.
# - 20s delay between targets; downloads up to MAX_PER_TARGET captures.
# - Per-target: CDX exact first, then CDX prefix if exact had <2 hits.

set -u
WORKDIR="$(cd "$(dirname "$0")/.." && pwd)/assets/sources/archive/wayback"
mkdir -p "$WORKDIR"
LOG="$WORKDIR/_wayback_hunt_log.txt"
...
TARGETS=(
  "https://www.artforum.com/print/200108|Artforum-2001-10-issue-TOC.html"
  "https://www.artforum.com/inprint/id=1672|Artforum-2001-10-Tartt-EgglestonPortfolio-candidate1.html"
  ...
  "https://oxfordamerican.org/magazine/issue-41-fall-2001|OA-Issue41-2001-TOC.html"
  ...
  "https://oxfordamerican.org/magazine/issue-29-september-october-1999|OA-Issue29-1999-TOC.html"
  "https://oxfordamerican.org/magazine/issue-6-march-april-1995/in-melbourne|OA-Issue6-1995-Tartt-InMelbourne.html"
  "https://oxfordamerican.org/magazine/issue-6-march-april-1995|OA-Issue6-1995-TOC.html"
  "https://www.gq.com/story/donna-tartt-garter-snake|GQ-1995-05-Tartt-AGarterSnake.html"
  "https://www.gq.com/contributor/donna-tartt|GQ-Tartt-contributor-page.html"
  "https://charlierose.com/episode/donna-tartt|CharlieRose-Tartt-episode-page.html"
  "https://charlierose.com/videos/donna-tartt|CharlieRose-Tartt-videos-page.html"
  "https://charlierose.com/show/donna-tartt|CharlieRose-Tartt-show-page.html"
  "https://www.mauritshuis.nl/en/our-collection/pen-meets-paint|Mauritshuis-PenMeetsPaint-Tartt-contribution.html"
  "https://www.anothermag.com/fashion-beauty/14523/sander-lak-interview-the-colours-of-sies-marjan-rizzoli|AnOther-SanderLak-SiesMarjan-Tartt-mention.html"
)

echo "Wayback hunt v3 — $(date)" > "$LOG"
...
# cdx_query <url> <exact|prefix>
# Emits "<timestamp>\t<original>" lines on stdout for status-200 captures.
# Retries 3x on non-200 or non-JSON. Diagnostic noise goes to stderr.
cdx_query() {
  local url="$1" mode="$2"
  local enc extra api
  enc=$(urlencode "$url")
...
# fetch_snapshot <ts> <orig_url> <outpath>
# Strictly rejects non-14-digit timestamps so we never download
# https://web.archive.org/web/<garbage>id_/... (which serves the Wayback homepage).
fetch_snapshot() {
...
for entry in "${TARGETS[@]}"; do
  ...
  # --- Phase 2: CDX prefix (only if exact returned <2) ---
  if [ "$hits" -lt 2 ]; then
    echo "  [cdx prefix]" >> "$LOG"
    while IFS=$'\t' read -r TS ORIG2; do
      [ -z "${TS:-}" ] && continue
